{
  new(
    ...
    local base = self,
    Properties: {
      assert std.isObject(HealthCheckConfig) : 'HealthCheckConfig must be an object',
  ...
  withHealthCheckTags(HealthCheckTags): {
    Properties+::: {
      HealthCheckTags: (if std.isArray(HealthCheckTags) then HealthCheckTags else [HealthCheckTags]),
    },
  },
