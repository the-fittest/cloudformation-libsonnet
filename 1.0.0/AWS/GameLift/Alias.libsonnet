{
  new(
    ...
    Properties: {
      ...
      RoutingStrategy:
        if !std.isObject(RoutingStrategy) then (error 'RoutingStrategy must be an object')
        else if !std.objectHas(RoutingStrategy, 'Type') then (error ' have attribute Type')
        else RoutingStrategy,
    },
    DependsOn:: [],
    CreationPolicy:: [],
    DeletionPolicy:: [],
