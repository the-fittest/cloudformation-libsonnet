{
  ...
  setContactFlowModuleArn(ContactFlowModuleArn): {
    Properties+::: {
      ContactFlowModuleArn:
        if !std.isString(ContactFlowModuleArn) then (error 'ContactFlowModuleArn must be a string')
        else if std.isEmpty(ContactFlowModuleArn) then (error 'ContactFlowModuleArn must be not empty')
        else if std.length(ContactFlowModuleArn) < 1 then error ('ContactFlowModuleArn should have at least 1 characters')
        else if std.length(ContactFlowModuleArn) > 256 then error ('ContactFlowModuleArn should have not more than 256 characters')
        else ContactFlowModuleArn,
    },
  },
  ...
  setState(State): {
    Properties+::: {
      State:
        ...
        else if std.length(State) > 500 then error ('State should have not more than 500 characters')
        else State,
    },
  },
  setStatus(Status): {
    Properties+::: {
      Status:
        ...
        else if std.length(Status) > 500 then error ('Status should have not more than 500 characters')
        else Status,
    },
  },
  setTags(Tags): {
