{
  new(
    ...
    Properties: {
      Type:
        ...
        else if Type != 'Sidewalk' && Type != 'LoRaWAN' then (error "Type should be 'Sidewalk' or 'LoRaWAN'")
        else Type,
      DestinationName:
        ...
        else if std.length(DestinationName) > 128 then error ('DestinationName should have not more than 128 characters')
  ...
  setPositioning(Positioning): {
    Properties+::: {
      Positioning:
        if !std.isString(Positioning) then (error 'Positioning must be a string')
        else if std.isEmpty(Positioning) then (error 'Positioning must be not empty')
        else if Positioning != 'Enabled' && Positioning != 'Disabled' then (error "Positioning should be 'Enabled' or 'Disabled'")
        else Positioning,
    },
  },
