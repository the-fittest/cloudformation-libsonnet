{
  new(
    ...
    Properties: {
      ...
      StoreFormat:
        if !std.isString(StoreFormat) then (error 'StoreFormat must be a string')
        else if std.isEmpty(StoreFormat) then (error 'StoreFormat must be not empty')
        else if StoreFormat != 'GFF' && StoreFormat != 'TSV' && StoreFormat != 'VCF' then (error "StoreFormat should be 'GFF' or 'TSV' or 'VCF'")
        else StoreFormat,
    },
    DependsOn:: [],
    CreationPolicy:: [],
