  ...
  new(
    ...
    Properties: {
      ...
      TargetArn:
        ...
        else if std.length(TargetArn) < 20 then error ('TargetArn should have at least 20 characters')
        else if std.length(TargetArn) > 2048 then error ('TargetArn should have not more than 2048 characters')
        else TargetArn,
      Rules:
        if !std.isArray(Rules) then (error 'Rules must be an array')
        else if std.length(Rules) < 1 then error ('Rules cannot have less than 1 items')
        else Rules,
    },
    DependsOn:: [],
    CreationPolicy:: [],
