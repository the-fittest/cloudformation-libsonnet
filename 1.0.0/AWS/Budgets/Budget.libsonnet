{
  new(
    ...
    local base = self,
    Properties: {
      assert std.isObject(Budget) : 'Budget must be an object',
      Budget: Budget,
    },
    DependsOn:: [],
    CreationPolicy:: [],
  ...
  withNotificationsWithSubscribers(NotificationsWithSubscribers): {
    Properties+::: {
      NotificationsWithSubscribers: (if std.isArray(NotificationsWithSubscribers) then NotificationsWithSubscribers else [NotificationsWithSubscribers]),
    },
  },
