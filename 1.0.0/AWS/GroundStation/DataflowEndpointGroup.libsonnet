{
  new(
    ...
    Properties: {
      EndpointDetails:
        if !std.isArray(EndpointDetails) then (error 'EndpointDetails must be an array')
        else if std.length(EndpointDetails) < 1 then error ('EndpointDetails cannot have less than 1 items')
