{
  ...
  setProvisioning(Provisioning): {
    Properties+::: {
      Provisioning:
        if !std.isString(Provisioning) then (error 'Provisioning must be a string')
        else if std.isEmpty(Provisioning) then (error 'Provisioning must be not empty')
        else if Provisioning != 'CUSTOMER_MANAGED' then (error "Provisioning should be 'CUSTOMER_MANAGED'")
        else Provisioning,
    },
  },
  setTags(Tags): {
