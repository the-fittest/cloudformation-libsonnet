{
  new(
    ...
    Properties: {
      ...
      FilterAction:
        if !std.isString(FilterAction) then (error 'FilterAction must be a string')
        else if std.isEmpty(FilterAction) then (error 'FilterAction must be not empty')
        else if FilterAction != 'NONE' && FilterAction != 'SUPPRESS' then (error "FilterAction should be 'NONE' or 'SUPPRESS'")
        else FilterAction,
    },
    DependsOn:: [],
    CreationPolicy:: [],
